...
namespace ohm_tsd_slam
{
  ...
  ThreadLocalize::ThreadLocalize(obvious::TsdGrid *grid, ThreadMapping *mapper, const double xOffset, const double yOffset, const std::string &nameSpace)
      : ThreadSLAM(*grid), _nameSpace(nameSpace), _mapper(*mapper), _sensor(nullptr), _initialized(false), _gridWidth(grid->getCellsX() * grid->getCellSize()), _gridHeight(grid->getCellsY() * grid->getCellSize()), _gridOffSetX(-1.0 * (grid->getCellsX() * grid->getCellSize() * 0.5 + xOffset)), _gridOffSetY(-1.0 * (grid->getCellsY() * grid->getCellSize() * 0.5 + yOffset)), _offsetInitial(Eigen::Vector2d(xOffset, yOffset)), _stampLaser(ros::Time::now()), _modelCoords(nullptr), _modelNormals(nullptr), _maskM(nullptr), _maskS(nullptr), _lastPose(std::make_unique<obvious::Matrix>(3, 3)), _rayCaster(std::make_unique<obvious::RayCastPolar2D>()),
        _reverseScan(false)
  {
  ...
  void ThreadLocalize::init(const sensor_msgs::LaserScan &scan)
  {
    ...
    prvNh.param<std::string>(_nameSpace + "file_config_robot", fileConfigRobot, "/home/user/workspace/ros/src/ohm_tsd_slam/config/config_robot_default.xml");
